...
# init 写好 projects.json 后，日常循环仍然用 dev-loop run。
cd /path/to/product-repo
dev-loop run --cli codex --agents core --once --dry-run
```
...
- **不安装插件时：**用 `dev-loop init-config` 从 npm 包内置模板创建
  `~/.claude/plugins/data/dev-loop/projects.json`，填写项目 key、`repoPath` 或 `repos[]`、
...
# 双层 Dev：senior-dev 设计，junior-dev 实现。
...
`--agents core` 表示 `pm,qa,dev,sweep`。可以再加 `reflect`、`outward` 或单个智能体：
